...
HEX = HEX
	or pcall(string_format, "%x", 2 ^ 31) and function(x) -- returns string of 8 lowercase hexadecimal digits
...
if branch == "FFI" then
	...
	function sha256_feed_64(H, str, offs, size)
		...
		for pos = offs, offs + size - 1, 64 do
			...
			for j = 0, 63, 8 do -- Thanks to Peter Cawley for this workaround (unroll the loop to avoid "PHI shuffling too complex" due to PHIs overlap)
				local z = NORM(
					XOR(g, AND(e, XOR(f, g))) + XOR(ROR(e, 6), ROR(e, 11), ROL(e, 7)) + (W[j] + K[j + 1] + h)
				)
				h, g, f, e = g, f, e, NORM(d + z)
				d, c, b, a =
					c, b, a, NORM(XOR(AND(a, XOR(b, c)), AND(b, c)) + XOR(ROR(a, 2), ROR(a, 13), ROL(a, 10)) + z)
	...
	function sha1_feed_64(H, str, offs, size)
		...
		for pos = offs, offs + size - 1, 64 do
			...
			for j = 0, 19, 5 do
				e, d, c, b, a =
					d, c, ROR(b, 2), a, NORM(ROL(a, 5) + XOR(d, AND(b, XOR(d, c))) + (W[j] + 0x5A827999 + e)) -- constant = floor(2^30 * sqrt(2))
				e, d, c, b, a =
					d, c, ROR(b, 2), a, NORM(ROL(a, 5) + XOR(d, AND(b, XOR(d, c))) + (W[j + 1] + 0x5A827999 + e))
				e, d, c, b, a =
					d, c, ROR(b, 2), a, NORM(ROL(a, 5) + XOR(d, AND(b, XOR(d, c))) + (W[j + 2] + 0x5A827999 + e))
				e, d, c, b, a =
					d, c, ROR(b, 2), a, NORM(ROL(a, 5) + XOR(d, AND(b, XOR(d, c))) + (W[j + 3] + 0x5A827999 + e))
				e, d, c, b, a =
					d, c, ROR(b, 2), a, NORM(ROL(a, 5) + XOR(d, AND(b, XOR(d, c))) + (W[j + 4] + 0x5A827999 + e))
			end
			for j = 20, 39, 5 do
			...
			H[1], H[2], H[3], H[4], H[5] =
				NORM(a + H[1]), NORM(b + H[2]), NORM(c + H[3]), NORM(d + H[4]), NORM(e + H[5])
		end
	end
end
		...
		function sha512_feed_128(H_lo, H_hi, str, offs, size)
			...
			for pos = offs, offs + size - 1, 128 do
				...
				for j = 1, 80 do
					...
					local z_lo, z_hi =
						NORM(sum_lo), NORM(u_hi + t_hi + h_hi + K_hi[j] + W[2 * j - 1] + floor(sum_lo / 2 ^ 32))
	...
	function md5_feed_64(H, str, offs, size)
		...
		for pos = offs, offs + size - 1, 64 do
			...
			for j = 17, 32, 4 do
				...
				a, d, c, b =
					d, c, b, NORM(ROL(XOR(c, AND(d, XOR(b, c))) + (K[j + 1] + W[AND(g + 5, 15) + 1] + a), 9) + b)
				a, d, c, b =
					d, c, b, NORM(ROL(XOR(c, AND(d, XOR(b, c))) + (K[j + 2] + W[AND(g + 10, 15) + 1] + a), 14) + b)
				a, d, c, b =
					d, c, b, NORM(ROL(XOR(c, AND(d, XOR(b, c))) + (K[j + 3] + W[AND(g - 1, 15) + 1] + a), 20) + b)
			end
			for j = 33, 48, 4 do
				local g = 3 * j + 2
	...
	function sha512_feed_128(H_lo, H_hi, str, offs, size)
		...
		for pos = offs, offs + size - 1, 128 do
			...
			for j = 1, 80 do
				...
				b_hi = a_hi
				tmp1 = z_lo
					+ (AND(d_lo, c_lo) + AND(b_lo, XOR(d_lo, c_lo))) % 4294967296
...
local function pad_and_xor(str, result_length, byte_for_xor)
	...
	end) .. string_rep(char(byte_for_xor), result_length - #str)
end
